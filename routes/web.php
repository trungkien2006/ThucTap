...
Route::middleware(['auth'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::get('/', function () {
        ...
        $deltas = [
            ...
            'speakers' => $getGrowthDelta(
                \App\Models\Speaker::whereYear('created_at', now()->year)->count(),
                \App\Models\Speaker::whereYear('created_at', now()->subYear()->year)->count()
            ),
            'media' => $getGrowthDelta(
                \App\Models\EventMedia::whereYear('created_at', now()->year)->count(),
                \App\Models\EventMedia::whereYear('created_at', now()->subYear()->year)->count()
            ),
        ];

        // 2. Events trend (last 12 months)
        $eventsTrend = [];
        for ($m = 1; $m <= 12; $m++) {
        ...
        // 3. Popular categories (Top 6)
        $popularCategories = \App\Models\Category::where('type', 'event_type')
