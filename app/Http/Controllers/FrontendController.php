...
class FrontendController extends Controller
{
    ...
    public function submitContact(Request $request)
    {
        ...
        try {
            $adminEmail = env('MAIL_FROM_ADDRESS', env('MAIL_USERNAME', 'admin@example.com'));
            
            // 1. Gửi email đẹp cho Admin
            $adminMailable = new \App\Mail\ContactNotification(
                $validated['name'],
                $validated['email'],
                $validated['subject'],
                $validated['message'],
            ...
            $adminMailable->replyTo($validated['email'], $validated['name']);
            Mail::to($adminEmail)->send($adminMailable);
            
            // 2. Gửi email xác nhận đẹp cho Khách hàng
            $userMailable = new \App\Mail\ContactNotification(
                $validated['name'],
                $validated['email'],
                $validated['subject'],
                $validated['message'],
            ...
            Mail::to($validated['email'])->send($userMailable);
