...
<head>
    ...
    <title>UniEvents | Chọn mẫu thiết kế — {{ $event->title }}</title>

    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link
        href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700&family=Hanken+Grotesk:wght@600;700&display=swap"
        rel="stylesheet">
    ...
    <style>
        ...
        .tp-iframe-wrap.mobile {
            max-width: 375px;
        }

        .tp-iframe-wrap.tablet {
            max-width: 768px;
        }
        ...
        /* Disable pointer events in preview to avoid scrolling hijacking */
...
<body class="font-sans antialiased text-slate-800">
    ...
    <!-- Fixed Header -->
    <header
        class="fixed top-0 left-0 w-full h-[64px] z-40 flex items-center justify-between px-6 bg-white border-b border-slate-200 shadow-sm">
        <div class="flex items-center gap-2 w-[240px]">
            <a href="{{ route('admin.events.index') }}" class="flex items-center gap-2">
                <div
                    class="w-8 h-8 rounded-lg bg-primary flex items-center justify-center text-white font-bold text-lg font-heading">
                    U</div>
        ...
        <div class="flex items-center gap-3">
            <div class="hidden md:flex items-center gap-2">
                <a href="{{ route('admin.events.edit', $event) }}"
                    class="text-[12px] text-slate-400 hover:text-primary transition-colors">① Thông tin</a>
        ...
        <div class="flex items-center justify-end w-[240px]">
            <a href="{{ route('admin.events.design', $event) }}"
                class="flex items-center gap-1 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-[13px] font-medium transition-all">
                Bỏ qua
    ...
    <div class="tp-layout">
            ...
            <div class="tp-sidebar-nav">
                <button class="tp-cat-btn active">
                    <span class="flex items-center gap-2"><span
                            class="material-symbols-outlined text-[18px]">grid_view</span> Tất cả mẫu</span>
                    <span
                        class="text-[11px] bg-white border border-orange-200 px-1.5 rounded-full text-orange-600">2</span>
                </button>
                @foreach($categories as $cat)
                    <button class="tp-cat-btn">
                        <span class="flex items-center gap-2"><span
                                class="material-symbols-outlined text-[18px]">folder</span> {{ $cat->name }}</span>
                    </button>
                @endforeach
            </div>
        </aside>
        ...
        <main class="tp-main">
            <div class="tp-main-header">
                ...
                <div class="flex items-center gap-2 bg-slate-100 p-1 rounded-lg border border-slate-200">
                    <button onclick="setGridLayout(1)"
                        class="w-8 h-8 flex items-center justify-center rounded text-slate-500 hover:bg-white hover:shadow-sm transition-all"><span
                            class="material-symbols-outlined text-[20px]">splitscreen</span></button>
                    <button onclick="setGridLayout(2)"
                        class="w-8 h-8 flex items-center justify-center rounded bg-white shadow-sm text-brand-orange transition-all"><span
                            class="material-symbols-outlined text-[20px]">grid_view</span></button>
                    <button onclick="setGridLayout(3)"
                        class="w-8 h-8 flex items-center justify-center rounded text-slate-500 hover:bg-white hover:shadow-sm transition-all"><span
                            class="material-symbols-outlined text-[20px]">apps</span></button>
                </div>
            </div>
            ...
            <div class="tp-grid-scroll">
                <div class="tp-grid cols-2" id="templateGrid">
                    ...
                    <!-- Template 1 -->
                    <div class="tp-card {{ ($event->page_template == 1 || $event->page_template == null) ? 'active' : '' }}"
                        id="card-1" onclick="selectTemplate(1)" style="cursor: pointer;">
                        <img src="https://images.unsplash.com/photo-1497366216548-37526070297c?auto=format&fit=crop&w=800&q=80"
                            alt="Template 1" class="tp-card-thumb">
                        <div class="tp-card-body">
                            <div class="flex justify-between items-start mb-1">
                                <h3 class="tp-card-title">Mẫu 1: Tiêu chuẩn</h3>
                                @if($event->page_template == 1 || $event->page_template == null)
                                    <span
                    ...
                    <!-- Template 2 -->
                    <div class="tp-card {{ $event->page_template == 2 ? 'active' : '' }}" id="card-2"
                        onclick="selectTemplate(2)" style="cursor: pointer;">
                        <img src="https://images.unsplash.com/photo-1518531933037-91b2f5f229cc?auto=format&fit=crop&w=800&q=80"
                            alt="Template 2" class="tp-card-thumb">
                        <div class="tp-card-body">
                            <div class="flex justify-between items-start mb-1">
                            ...
                            <p class="tp-card-desc">Phong cách thanh lịch, tông màu tự nhiên, lý tưởng cho sự kiện văn
                                hóa.</p>
                            <div class="tp-card-actions" onclick="event.stopPropagation()">
                                <form action="{{ route('admin.events.save_template', $event) }}" method="POST"
                                    class="flex-1">
        ...
        <aside class="tp-preview-panel" id="previewPanel">
            ...
            <div class="tp-preview-header">
                <span class="font-bold text-[14px] flex items-center gap-2"><span
                        class="material-symbols-outlined text-[18px]">visibility</span> Xem thử mẫu</span>
                <div class="flex bg-slate-100 rounded-lg p-1">
                    <button onclick="setPreviewDevice('desktop')"
                        class="w-7 h-7 rounded flex items-center justify-center bg-white shadow-sm text-primary transition-all"><span
                            class="material-symbols-outlined text-[16px]">desktop_windows</span></button>
                    <button onclick="setPreviewDevice('tablet')"
                        class="w-7 h-7 rounded flex items-center justify-center text-slate-500 hover:bg-white hover:shadow-sm transition-all"><span
                            class="material-symbols-outlined text-[16px]">tablet_mac</span></button>
                    <button onclick="setPreviewDevice('mobile')"
                        class="w-7 h-7 rounded flex items-center justify-center text-slate-500 hover:bg-white hover:shadow-sm transition-all"><span
                            class="material-symbols-outlined text-[16px]">smartphone</span></button>
                </div>
            </div>
            <div class="tp-preview-content">
                <div class="tp-iframe-wrap desktop" id="previewWrapper">
                    <iframe id="miniPreviewFrame"
                        src="{{ route('admin.events.template_preview', $event->page_template ?? 1) }}"
                        class="tp-iframe"></iframe>
                </div>
            </div>
            <div class="p-4 border-t border-slate-200 bg-white">
                <form action="{{ route('admin.events.save_template', $event) }}" method="POST">
                    @csrf
                    <input type="hidden" name="page_template" id="formCurrentTemplateId"
                        value="{{ $event->page_template ?? 1 }}">
                    <button type="submit"
                        class="w-full py-2.5 bg-primary hover:bg-slate-800 text-white font-semibold rounded-xl text-[13px] shadow transition-all flex items-center justify-center gap-2">
        ...
        <div class="fp-header">
            ...
            <div class="flex items-center gap-4">
                <form action="{{ route('admin.events.save_template', $event) }}" method="POST">
                    ...
                    <button type="submit"
                        class="px-4 py-2 bg-brand-orange hover:bg-orange-600 text-white font-semibold rounded-lg text-[13px] transition-all">
                        Chọn mẫu này
                    </button>
                </form>
                <button onclick="closeFullPreview()"
                    class="w-10 h-10 flex items-center justify-center text-slate-300 hover:text-white bg-white/10 hover:bg-white/20 rounded-full transition-all">
    ...
    <script>
        ...
        function previewTemplate(id) {
            ...
            document.getElementById('fullPreviewFrame').src = "{{ url('/admin/template-preview') }}/" + id;
            document.getElementById('fpTemplateId').value = id;
            let templateName = 'Mẫu 1: Tiêu chuẩn';
            if (id === 2) templateName = 'Mẫu 2: Garden';
            if (id === 3) templateName = 'Mẫu 3: AIFuture (Mint Green)';
