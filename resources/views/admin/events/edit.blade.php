...
<div class="max-w-[1000px] mx-auto">
    ...
    <form action="{{ route('admin.events.update', $event) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <input type="hidden" name="has_guests_field" value="1">
...
<!-- Section 5: Guests -->
            ...
            <div>
                ...
                <div id="guest_list_container" class="grid grid-cols-1 md:grid-cols-2 gap-3 max-h-[240px] overflow-y-auto p-2 border border-slate-200 rounded-xl bg-slate-50/50">
                    @forelse($guests as $guest)
                        ...
                        <label class="flex items-center justify-between p-3 bg-white border border-slate-200 rounded-lg cursor-pointer hover:border-brand-orange transition-all relative">
                            <div class="flex items-center gap-3">
                                <input type="checkbox" name="guest_ids[]" value="{{ $guest->id }}" {{ (is_array(old('guest_ids')) && in_array($guest->id, old('guest_ids'))) || (!old('guest_ids') && $event->guests->contains($guest->id)) ? 'checked' : '' }} class="rounded border-slate-300 text-brand-orange focus:ring-brand-orange w-4 h-4">
                                <div class="flex items-center gap-3">
                                    <img src="{{ $photoUrl }}" class="w-8 h-8 rounded-full object-cover">
                                    <div>
        ...
        <section class="uni-card p-6 space-y-5">
            ...
            <div id="preview_container" class="{{ $hasBanner ? '' : 'hidden' }} rounded-xl overflow-hidden border border-slate-200 h-[320px] mb-4">
                <img id="banner_preview" src="{{ $bannerUrl }}" alt="Preview" class="w-full h-full object-cover">
            </div>
