...
@section('content')
    ...
    <section id="top" class="relative h-[100svh] w-full overflow-hidden" style="background:#1C1410;">
        ...
        <div class="slider-wrapper" id="slider">
            ...
            <div class="slider-overlay"
                style="background:linear-gradient(110deg,rgba(255,200,60,0.50) 0%,rgba(28,20,16,0.55) 50%,rgba(7,160,195,0.15) 100%);">
            </div>
            <div class="slider-content">
                <div class="slide-info is-active" id="slideInfo">
                    ...
                    <div class="slide-actions">
                        ...
                        <a href="{{ $slides[0]['cta_url'] }}" class="btn-cta"
                            id="slideCta">{{ $slides[0]['cta_label'] }}</a>
                    </div>
                </div>
    ...
    <section class="relative overflow-hidden" style="background:#FFE381;">
        <div class="pointer-events-none absolute inset-0">
            <div class="absolute right-0 top-0 h-64 w-64 rounded-full opacity-20 blur-3xl" style="background:#07A0C3;">
            </div>
            <div class="absolute left-0 bottom-0 h-48 w-48 rounded-full opacity-15 blur-2xl" style="background:#04F06A;">
            </div>
        </div>
    ...
    <section id="events" class="relative py-24 lg:py-32" style="background:#FFFBEA;">
        ...
        <div class="mx-auto grid max-w-[1400px] grid-cols-1 gap-12 px-6 lg:grid-cols-[1fr_360px] lg:gap-14 lg:px-10">
            ...
            <div>
                <div data-aos="fade-up" class="mb-10 flex items-end justify-between">
                    <div>
                        ...
                        <h2
                            class="font-['Barlow_Condensed'] text-5xl font-black uppercase tracking-tight text-[#1C1410] lg:text-6xl">
                            Sự kiện nổi bật</h2>
                    </div>
                ...
                <div class="space-y-5">
                    @foreach($featuredEvents as $i => $ev)
                        <article data-aos="fade-up" data-aos-delay="{{ $i * 60 }}" x-data="{ animClass: '' }"
                            @mouseenter="animClass = 'hover-anim-' + (Math.floor(Math.random() * 3) + 1)"
                            @mouseleave="animClass = ''" :class="animClass" class="group relative grid grid-cols-1 overflow-hidden rounded-2xl transition-all duration-500
                            ...
                            <div class="relative z-10 h-52 overflow-hidden sm:h-full">
                                ...
                                <div class="absolute left-3 top-3">
                                    <span
                                        class="rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-wide text-[#1C1410] shadow-md"
                            ...
                            <div class="relative z-10 flex flex-col justify-between p-6">
                                <div>
                                    <h3
                                        class="font-['Barlow_Condensed'] text-3xl font-black uppercase leading-tight tracking-wide text-[#1C1410] transition-colors group-hover:text-[#07A0C3] lg:text-4xl">
                                        {{ $ev['title'] }}
                                    </h3>
                                    <div class="mt-3 flex flex-wrap gap-x-5 gap-y-2 text-sm text-[#7A6A52]">
                                        <span class="inline-flex items-center gap-2">
                                            <i data-lucide="calendar" class="h-4 w-4" style="color:#07A0C3;"></i>
                                            {{ $ev['date'] }}
                                        </span>
                                        <span class="inline-flex items-center gap-2">
                                            <i data-lucide="map-pin" class="h-4 w-4" style="color:#04F06A;"></i>
                                            {{ $ev['location'] }}
                                        </span>
                                    </div>
                                </div>
                                <div class="mt-5 flex items-center gap-4">
                                    ...
                                    <span
                                        class="text-xs font-semibold uppercase tracking-wider text-[#7A6A52]">{{ $ev['category'] }}</span>
                                </div>
                            </div>
                        </article>
                    @endforeach
            ...
            <aside class="lg:sticky lg:top-28 lg:self-start">
                <div data-aos="fade-left" class="overflow-hidden rounded-2xl shadow-xl" style="border:2px solid #FFE381;">
                    {{-- Header Jasmine --}}
                    <div class="relative overflow-hidden px-6 py-6" style="background:#FFE381;">
                        <div class="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full opacity-30"
                            style="background:#07A0C3;"></div>
                        <div class="relative flex items-center justify-between">
                            <div>
                                <div class="text-xs font-bold uppercase tracking-[0.25em] text-[#7A6A52]">Upcoming</div>
                                <h3
                                    class="font-['Barlow_Condensed'] mt-0.5 text-3xl font-black uppercase tracking-wide text-[#1C1410]">
                                    Sắp diễn ra</h3>
                            </div>
                            <div
                                class="flex items-center gap-2 rounded-full bg-white/60 px-3 py-1.5 text-xs font-bold text-[#1C1410] backdrop-blur">
                    ...
                    <div class="p-6" style="background:#FFFBEA;">
                        <ol class="relative space-y-4 border-l-2 border-[#FFE381] pl-5">
                            @foreach($upcoming as $i => $u)
                                <li data-aos="fade-right" data-aos-delay="{{ $i * 80 }}"
                                    class="relative group/item cursor-pointer">
                                    <span
                                        class="absolute -left-[25px] top-1.5 h-3 w-3 rounded-full border-2 border-[#FFFBEA] shadow transition-transform group-hover/item:scale-125"
                                        style="{{ $u['open'] ? 'background:#07A0C3;' : 'background:#d1c9a8;' }}"></span>
                                    <div class="text-[11px] font-bold uppercase tracking-widest text-[#7A6A52]">{{ $u['date'] }}
                                    </div>
                                    <div
                                        class="mt-0.5 text-sm font-semibold text-[#1C1410] group-hover/item:text-[#07A0C3] transition-colors">
                                        {{ $u['name'] }}</div>
    ...
    <section id="archive" class="relative overflow-hidden py-24 lg:py-32"
        ...
        <div class="pointer-events-none absolute inset-0 overflow-hidden">
            <div class="absolute -left-32 top-1/4 h-[450px] w-[450px] rounded-full blur-[130px] opacity-25"
                style="background:#FFE381;"></div>
            <div class="absolute -right-32 bottom-10 h-[350px] w-[350px] rounded-full blur-[130px] opacity-20"
                style="background:#07A0C3;"></div>
            <div class="absolute left-1/2 bottom-0 h-40 w-[600px] -translate-x-1/2 rounded-full blur-[80px] opacity-15"
                style="background:#04F06A;"></div>
        </div>
        <!-- Top border Jasmine -->
        ...
        <div class="relative mx-auto max-w-[1400px] px-6 lg:px-10">
            <div data-aos="fade-up" class="flex items-end justify-between">
                <div>
                    ...
                    <h2
                        class="font-['Barlow_Condensed'] text-5xl font-black uppercase tracking-tight text-white lg:text-7xl">
                        Kho lưu trữ sự kiện</h2>
            ...
            <div class="relative mt-14 grid grid-cols-1 gap-10 lg:grid-cols-[0.9fr_1.1fr] lg:gap-16">
                ...
                <div class="relative flex items-start">
                    <div class="font-['Barlow_Condensed'] text-[28vw] font-black leading-[0.85] tracking-tighter lg:text-[18vw]"
                        ...
                        x-text="current.year" x-transition:enter="transition ease-out duration-500"
                ...
                <div>
                    <div class="group relative h-[320px] overflow-hidden rounded-2xl lg:h-[420px]"
                        ...
                        <div class="absolute inset-0"
                            style="background:linear-gradient(to top,rgba(45,31,10,0.92) 0%,rgba(45,31,10,0.25) 60%,transparent 100%);">
                        </div>
                        <!-- Jasmine accent bar -->
                    ...
                    <p class="mt-5 text-base leading-relaxed lg:text-lg" style="color:rgba(255,227,129,0.7);"
                        x-text="current.desc"></p>
                    ...
                    <div class="mt-7 flex flex-wrap items-center gap-2">
                        <template x-for="(a,i) in archive" :key="a.year">
                            <button @click="dir=i>idx?1:-1;idx=i"
                                class="rounded-full px-4 py-1.5 text-sm font-bold font-mono transition-all" :style="i===idx
    ...
    <section class="relative overflow-hidden py-24 lg:py-32" style="background:#FFF3C4;">
        ...
        <!-- Subtle accent blobs -->
        <div class="pointer-events-none absolute inset-0 overflow-hidden">
            <div class="absolute right-0 top-0 h-80 w-80 rounded-full blur-[120px] opacity-30" style="background:#07A0C3;">
            </div>
            <div class="absolute -left-20 bottom-0 h-64 w-64 rounded-full blur-[100px] opacity-20"
                style="background:#04F06A;"></div>
        </div>
        ...
        <div class="relative mx-auto max-w-[1400px] px-6 lg:px-10">
            <div data-aos="fade-up" class="flex items-end justify-between pb-10">
                <div>
                    ...
                    <h2
                        class="font-['Barlow_Condensed'] text-4xl font-black uppercase tracking-tight text-[#1C1410] lg:text-5xl">
                        Album & Recap</h2>
                </div>
                <a href="#"
                    class="group inline-flex items-center gap-1.5 text-sm font-semibold text-[#07A0C3] transition-colors hover:text-[#04F06A]">
                    Thư viện đầy đủ <i data-lucide="arrow-right"
                    ...
                    <div data-aos="fade-up" data-aos-delay="{{ $i * 80 }}">
                        <a href="#"
                            ...
                            <img src="{{ $m['src'] }}" alt="{{ $m['label'] }}" loading="lazy"
                                class="h-full w-full object-cover transition-transform duration-700 group-hover:scale-110" />
                            <div class="absolute inset-0 bg-gradient-to-t from-[#2D1F0A]/80 via-[#2D1F0A]/10 to-transparent">
                            </div>
                            ...
                            <div class="absolute bottom-0 left-0 right-0 p-4">
                                ...
                                <div class="mb-2 h-0.5 w-0 rounded-full transition-all duration-500 group-hover:w-full"
                                    style="background:#FFE381;"></div>
                                <div class="text-[9px] font-bold uppercase tracking-[0.25em]" style="color:#FFE381;">
                                    {{ $m['type'] }}</div>
                                <div
                                    class="font-['Barlow_Condensed'] mt-1 text-lg font-bold uppercase tracking-wide text-white group-hover:text-[#FFE381] transition-colors">
                                    {{ $m['label'] }}</div>
                            </div>
                        </a>
                    </div>
    ...
    <style>
        ...
        .snowflake {
            ...
            top: -10%;
            user-select: none;
            animation-name: fall, sway, colorSize;
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function () {
            ...
            for (let i = 0; i < numSnowflakes; i++) {
                ...
                sf.textContent = chars[Math.floor(Math.random() * chars.length)];

                // Randomize positions and durations
                let left = Math.random() * 100;
                let fallDuration = 6 + Math.random() * 8; // 6s to 14s
                let swayDuration = 2 + Math.random() * 2; // 2s to 4s
                let colorDuration = 4 + Math.random() * 4; // 4s to 8s
                let fallDelay = Math.random() * 10;
                let colorDelay = Math.random() * 5;

                sf.style.left = left + 'vw';
                sf.style.animationDuration = `${fallDuration}s, ${swayDuration}s, ${colorDuration}s`;
                sf.style.animationDelay = `-${fallDelay}s, -${fallDelay}s, -${colorDelay}s`;
                sf.style.opacity = 0.7 + Math.random() * 0.3;
