<a href="{{ route('events.show', $ev->slug ?? $ev->id) }}" class="fab-event-card group block relative rounded-2xl overflow-hidden cursor-pointer shadow-sm border border-transparent transition-all hover:shadow-md hover:border-[#E8C84A] hover:scale-[1.02]">
    <!-- Image -->
    <div class="w-full aspect-[4/3] bg-slate-200 relative overflow-hidden">
        @if($ev->bannerImage)
            <img src="{{ \App\Helpers\FileHelper::url($ev->bannerImage->url) }}" class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110">
        @else
        ...
        <div class="absolute bottom-0 left-0 right-0 p-4">
            ...
            <div class="text-xs text-white/70 mt-1 font-medium tracking-widest uppercase">{{ $ev->event_date->format('d/m/Y') }}</div>
        </div>
    </div>
</a>
