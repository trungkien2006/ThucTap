                    ...
                    <div class="flex flex-wrap gap-2 max-h-[160px] overflow-y-auto p-2.5 border border-slate-200 rounded-xl bg-slate-50/50 mt-1">
                        @foreach($departments as $dept)
                            <label class="flex items-center gap-2 px-3 py-2 bg-white border border-slate-200 rounded-lg cursor-pointer hover:border-brand-orange transition-all">
                                <input type="checkbox" name="department_ids[]" value="{{ $dept->id }}" {{ (is_array(old('department_ids')) && in_array($dept->id, old('department_ids'))) || (!old('department_ids') && $event->departments->contains($dept->id)) ? 'checked' : '' }} class="rounded border-slate-300 text-brand-orange focus:ring-brand-orange w-4 h-4 shrink-0">
