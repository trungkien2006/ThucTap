    ...
    <div class="uni-card overflow-clip mb-8">
        ...
        <div class="bg-slate-50 px-4 py-2.5 border-b border-slate-100 flex items-center gap-3">
            ...
            <div class="flex-1 bg-white rounded-lg px-3 py-1 text-[11px] text-slate-400 border border-slate-200">
                <?php echo e(url('/events/' . $event->slug)); ?>

            </div>
            <a href="<?php echo e(route('events.show', $event->slug)); ?>" target="_blank" class="text-slate-400 hover:text-primary transition-colors" title="Mở trong tab mới">
                <span class="material-symbols-outlined text-[18px]">open_in_new</span>
            </a>
        </div>

        <!-- Preview Content -->
        <iframe src="<?php echo e(route('admin.events.preview_iframe', $event)); ?>" class="w-full h-[85vh] border-0 rounded-b-xl bg-white" title="Preview"></iframe>
    </div>

    <!-- Action Buttons -->
    <div class="flex items-center justify-between">
        <a href="<?php echo e(route('admin.events.design', $event)); ?>" class="btn-ghost flex items-center gap-2">
            <span class="material-symbols-outlined text-[18px]">arrow_back</span>
            Quay lại thiết kế
        </a>
        <div class="flex gap-3">
            <form action="<?php echo e(route('admin.events.update', $event)); ?>" method="POST" class="inline">
                <?php echo csrf_field(); ?>
                <?php echo method_field('PUT'); ?>
                <input type="hidden" name="title" value="<?php echo e($event->title); ?>">
                <input type="hidden" name="slug" value="<?php echo e($event->slug); ?>">
                <input type="hidden" name="description" value="<?php echo e($event->description); ?>">
                <input type="hidden" name="event_date" value="<?php echo e($event->event_date); ?>">
                <input type="hidden" name="location" value="<?php echo e($event->location); ?>">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\admin\Downloads\ThucTap-main\ThucTap-main\resources\views/admin/events/preview.blade.php ENDPATH**/ ?>