        ...
        <div class="bg-card rounded-2xl border-none shadow-2xl shadow-slate-300/60 hover:-translate-y-2 hover:shadow-slate-300/80 transition-all duration-300 p-5 flex flex-col gap-3 bg-card rounded-2xl border-none shadow-2xl shadow-slate-300/60 hover:-translate-y-2 hover:shadow-slate-300/80 transition-all duration-300-hover">
            <div class="flex items-start justify-between">
                ...
                <div class="flex items-center gap-1">
                    ...
                    <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" class="inline-block" onsubmit="return confirmDelete(event, this, 'Bạn có chắc chắn muốn xóa danh mục này? Hành động này không thể hoàn tác.');">
                        @csrf
                        @method('DELETE')
            ...
            <div class="flex items-center justify-between pt-2 border-t border-border">
                <div>
                    <div class="text-lg font-semibold tabular-nums">{{ $category->events_count }}</div>
                    <div class="text-[11px] text-muted-foreground">sự kiện</div>
                </div>
                <div class="text-right">
                    <div class="text-lg font-semibold tabular-nums">{{ number_format($category->total_views ?? 0) }}</div>
