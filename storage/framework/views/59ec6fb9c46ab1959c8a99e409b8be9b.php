...
<?php $__env->startSection('content'); ?>
    <div class="space-y-4">
        <div class="flex items-end justify-between flex-wrap gap-3">
            <div class="flex items-center gap-3">
                <a href="<?php echo e(route('admin.media.index')); ?>" class="w-10 h-10 rounded-xl border border-border flex items-center justify-center text-muted-foreground hover:bg-accent hover:text-foreground transition-all">
                    <i data-lucide="arrow-left" class="h-5 w-5"></i>
                </a>
                <div>
                    <h1 class="text-[22px] font-semibold tracking-tight">Album: <?php echo e($event->title); ?></h1>
                    <p class="text-xs text-muted-foreground mt-0.5"><?php echo e($media->total()); ?> tệp</p>
                </div>
            </div>
        ...
        <div class="grid grid-cols-[1fr_260px] md:grid-cols-[1fr_320px] lg:grid-cols-[1fr_400px] xl:grid-cols-[1fr_480px] gap-4">
            ...
            <div class="space-y-3">
                <form action="<?php echo e(route('admin.media.index')); ?>" method="GET" class="flex items-center gap-2 justify-end mb-4">
                    ...
                    <div class="relative flex items-center shrink-0">
                        <select name="sort" onchange="this.form.submit()"
                            ...
                            <option value="date_desc" <?php echo e(request('sort') === 'date_desc' ? 'selected' : ''); ?>>Mới nhất</option>
                            <option value="date_asc" <?php echo e(request('sort') === 'date_asc' ? 'selected' : ''); ?>>Cũ nhất</option>
                        </select>
                ...
                <div class="grid grid-cols-3 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 gap-2.5">
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $media; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $m): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoopIteration(); ?><?php endif; ?>
                        <div class="bg-card rounded-lg border border-border overflow-hidden shadow-sm hover:shadow-md transition-all group cursor-pointer media-item-card"
                            data-url="<?php echo e(\App\Helpers\FileHelper::url($m->url)); ?>" data-type="<?php echo e($m->type); ?>"
                            data-caption="<?php echo e($m->caption ?? basename($m->url)); ?>"
                            data-created-at="<?php echo e($m->created_at ? $m->created_at->diffForHumans() : '—'); ?>"
                            data-event="<?php echo e($m->event->title ?? '—'); ?>" data-delete-url="<?php echo e(route('admin.media.destroy', $m)); ?>">
                            <div
                            ...
                            <div class="p-2">
                                <div class="text-[11px] font-medium truncate"><?php echo e($m->caption ?? basename($m->url)); ?></div>
                                <div class="text-[10px] text-muted-foreground flex justify-between">
                                    <span><?php echo e(strtoupper($m->type)); ?></span>
                                    <span><?php echo e($m->created_at ? $m->created_at->diffForHumans() : '—'); ?></span>
                                </div>
                            </div>
                        </div>
                ...
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($media->hasPages()): ?>
                    <div class="flex justify-center mt-4">
                        <?php echo e($media->appends(request()->query())->links()); ?>
            ...
            <div class="bg-card rounded-lg border border-border p-3 shadow-sm h-fit">
                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($media->first()): ?>
                    ...
                    <div id="preview-media-container" onclick="openLightbox()"
                        ...
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($first->type === 'image'): ?>
                            <img src="<?php echo e(\App\Helpers\FileHelper::url($first->url)); ?>" class="w-full h-full object-cover rounded-md" alt="">
                        <?php elseif($first->type === 'video'): ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(str_contains($first->url, 'drive.google.com')): ?>
                                <iframe src="<?php echo e($first->url); ?>" class="w-full h-full rounded-md border-0" allow="autoplay" allowfullscreen></iframe>
                            <?php else: ?>
                                <video src="<?php echo e(\App\Helpers\FileHelper::url($first->url)); ?>" controls
                    ...
                    <div class="mb-3">
                        <div id="preview-caption" class="text-sm font-semibold truncate">
                            <?php echo e($first->caption ?? basename($first->url)); ?>

                        </div>
                        <div id="preview-type-badge" class="text-[11px] text-muted-foreground"><?php echo e(strtoupper($first->type)); ?>
                    ...
                    <div class="space-y-1.5 text-[11px] border-t border-border pt-2">
                        <div class="flex justify-between gap-2">
                            ...
                            <span id="preview-created-at"
                                class="font-medium truncate"><?php echo e($first->created_at ? $first->created_at->diffForHumans() : '—'); ?></span>
                        </div>
                        <div class="flex justify-between gap-2">
                            <span class="text-muted-foreground">Định dạng</span>
                            <span id="preview-format" class="font-medium"><?php echo e(strtoupper($first->type)); ?></span>
                        </div>
                        <div class="flex justify-between gap-2">
                            <span class="text-muted-foreground">Sự kiện</span>
                            <span id="preview-event" class="font-medium truncate"><?php echo e($first->event->title ?? '—'); ?></span>
                        </div>
                    </div>
                    <div class="flex gap-2 pt-3 border-t border-border mt-2">
                        <form id="preview-delete-form" action="<?php echo e(route('admin.media.destroy', $first)); ?>" method="POST"
    ...
    <div id="uploadMediaModal"
        ...
        <div class="bg-background border border-border rounded-xl shadow-lg w-full max-w-lg mx-4 p-6 relative">
            ...
            <form action="<?php echo e(route('admin.media.store')); ?>" method="POST" enctype="multipart/form-data" class="space-y-5">
                <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\kienxinhzai\Downloads\ThucTap-tuan\resources\views/admin/media/show.blade.php ENDPATH**/ ?>